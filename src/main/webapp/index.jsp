    ...
    <div id="tb" style="padding: 5px;">
        ...
        <a href="#" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-ok'" onclick="f_updateReadAllFlag();return false;" title="全部置为已读">全部置为已读</a>
			...
			searcher:function(value,name){if (name==1){f_getArticles({title:value});}else{f_getArticles({content:value});} }" style="width:200px"/>
                       ...
                       日期：
                       <input type="text" id="creattime1"  class="Wdate" size="10" onClick="WdatePicker({onpicked:function(dp){
                creattime=dp.cal.getDateStr();
                f_getArticles({creattime1:$('#creattime1').val(),creattime2:$('#creattime2').val()});
            }})" readonly="readonly" > 至
                <input type="text" id="creattime2"  class="Wdate" size="10" onClick="WdatePicker({onpicked:function(dp){
                creattime=dp.cal.getDateStr();
                f_getArticles({creattime1:$('#creattime1').val(),creattime2:$('#creattime2').val()});
            }})" readonly="readonly" >
