...
(function () {
    ...
    var tmp = window.location.pathname,
        ...
        URL=CONTEXT_PATH+"/js/ueditor/";
    UEDITOR_CONFIG = {
        imagePath:CONTEXT_PATH + "/", //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        compressSide:0,                   //等比压缩的基准，确定maxImageSideLength参数的参照对象。0为按照最长边，1为按照宽度，2为按照高度
        maxImageSideLength:4096,          //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值
        relativePath:true,                //是否开启相对路径。开启状态下所有本地图片的路径都将以相对路径形式进行保存.强烈建议开启！

        filePath:CONTEXT_PATH + "/",  //附件文件夹保存路径
	    catchRemoteImageEnable:false,                                   //是否开启远程图片抓取
        catcherUrl:URL +"server/upload/jsp/getRemoteImage.jsp",             //处理远程图片抓取的地址
        localDomain:"baidu.com",                                        //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地
	    imageManagerPath:URL + "server/upload/jsp/imageManager.jsp",       //图片在线浏览的处理地址
        ...
        snapscreenServerFile: URL +"server/upload/jsp/imageUp.jsp", //屏幕截图的server端保存程序，UEditor的范例代码为“URL +"server/upload/php/snapImgUp.php"”
        snapscreenServerPort: 8880,//屏幕截图的server端端口
